...
def main():
    ...
    # 创建基础 SamplingParams（新版本 vLLM 使用 StructuredOutputsParams）
    ...
    for pair_idx, file_pair in enumerate(INPUT_OUTPUT_PAIRS, 1):
        ...
        if not os.path.exists(input_path):
            print(f"⚠️  输入文件不存在，跳过: {input_path}")
            continue
        ...
        for i in range(start_index, total_items, BATCH_SIZE):
            ...
            batch_results = [None] * len(current_batch_items) # 预占位，用于填入结果
            ...
            # 2. 如果有有效数据，进行推理
            if valid_prompts:
                # print(f"  - Inferencing {len(valid_prompts)} items...")
            ...
            # 4. 将本批次结果添加到总结果
            # 注意：batch_results 里可能还有 None（如果逻辑写错），做个防御
            for res in batch_results:
                if res is None:
                    # 理论上不该进这里，除非代码逻辑漏洞，补个未知错误
        ...
        # 全部完成，保存最终文件
        print(f"\n✅ 文件 {output_path} 处理完成！最终保存到: {output_path}")
