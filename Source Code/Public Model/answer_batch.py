...
def main():
    ...
    "2) Output Structure: First output your thinking within </think> blocks, then provide the Answer.\n"
            ...
            item = current_batch_items[original_idx]
                ...
                try:
                    ...
                    if answer_idx != -1:
                        ...
                        llm_complexCOT = llm_complexCOT.replace('</think>', '').strip()
                        llm_complexCOT = llm_complexCOT.replace('<think>', '').strip()
                        llm_answer = lines[answer_idx].strip()[7:].strip()  # 去掉 "Answer:" 前缀
                    else:
